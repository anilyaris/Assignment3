	...
	selected_joint_			(0)
{
	...
	common_ctrl_.addToggle(&animationMode,							FW_KEY_3, "Animate mesh");
...
void App::initRendering()
{
	...
	auto ssd_prog = new GLContext::Program(
		...
		void main()
		{
			...
			for (int i = 0; i < 4; i++) {
				if (aJoints1[i] > 0) {
					W += aWeights1[i] * uJoints[aJoints1[i]];
				}
			}
			...
			float clampedCosine = clamp(dot(normalize(Wxyz * aNormal), directionToLight), 0.0, 1.0);
			vec3 litColor = vec3(clampedCosine);
			vColor = vec4(mix(aColor.xyz, litColor, uShadingMix), 1);
			gl_Position = uWorldToClip * W * aPosition;
		}
		),
		"#version 330\n"
		FW_GL_SHADER_SOURCE(
...
void App::render() {
	...
	if (animationMode)
	{
		int animFrame = (GetTickCount() - animationStart) / 16.666668f;
	...
	if (drawmode_ == MODE_SKELETON) {
		...
		glUniformMatrix4fv(gl_.ssd_transforms_uniform, JOINTS, GL_FALSE, ssd_transforms[0].getPtr());
...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		// This is very simple...
		Mat4f transforms_i = transforms[i];
		Vec3f joint_world_pos = transforms_i.getCol(3).toCartesian();
		...
		Mat3f invT = transforms_i.getXYZ().transposed().inverted();		
		/*Vec3f right(transforms_i.m00, transforms_i.m01, transforms_i.m02), 
			up(transforms_i.m10, transforms_i.m11, transforms_i.m12), 
			ahead(transforms_i.m20, transforms_i.m21, transforms_i.m22);*/
		Vec3f right = invT.getCol(0), up = invT.getCol(1), ahead = invT.getCol(2);
		...
		glColor3f(1, 0, 0); // red
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + right.x, joint_world_pos.y + right.y, joint_world_pos.z + right.z);

		// ..and the y axis.. ("up")
		glColor3f(0, 1, 0); // green
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + up.x, joint_world_pos.y + up.y, joint_world_pos.z + up.z);

		// ..and the z axis ("ahead").
		glColor3f(0, 0, 1); // blue
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
		glVertex3f(joint_world_pos.x + ahead.x, joint_world_pos.y + ahead.y, joint_world_pos.z + ahead.z);
		...
		int parent_index = skel_.getJointParent(i);
		if (parent_index != -1) {
			Vec3f parent_world_pos = transforms[parent_index].getCol(3).toCartesian();
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		for (int i = 0; i < 8; i++) {
			if (sv.joints[i] > 0) {
				W += sv.weights[i] * ssd_transforms[sv.joints[i]];				
			}
		}
		...
		for (int i = 0; i < 3; i++) {
			Vec4f V = W.getCol(i);
			Wxyz.setCol(i, V.getXYZ());
		}

		v.position = (W * sv.position.toHomogeneous()).toCartesian();
		v.normal = (Wxyz * sv.normal).normalized();
		v.color = sv.color;//(W * sv.color.toHomogeneous()).toCartesian();
...
vector<WeightedVertex> App::loadAnimatedMesh(string namefile, string mesh_file, string attachment_file) {
	...
	for (auto& v : vertices)
		v.position = (v.position - Vec3f(0, 97, 0)) / scale;
...
vector<WeightedVertex> App::loadWeightedMesh(string mesh_file, string attachment_file) {
	...
	while(getline(attachment_input, line)) {
		...
		stringstream ss(line);
		auto n_weights = 0u;
		for (auto i = 0u; i < JOINTS-1 && ss.good(); ++i) {
			float weight;
			ss >> weight;
			if (weight != 0) {
				temp_w[n_weights] = weight;
